...
- Webapp: http://162.55.213.88
Használt technológiák: Vue3 (Frontend), NodeJS 22 (Express backend API), Python 3.13 (Flask image processing API), OpenCV (Object Detection API), Docker (Compose), GitHub Actions, GitHub Container Registry
...
image::upload.png[title="Feltöltés" caption="{counter2:figure}{counter:figure}. {figure-caption} "]
...
- webpage-host: weboldalt kiszolgáló program (kész)
...
A file-store a 3000-es porton érhető el.
Az image-processor-nak nem kell kifelé elérhetőnek lennie így szintén a 3000-es porton működik alapból.
...
A Docker Image-ket a Docker Compose futtatja. Az image-ket a GHCR-ről szedi le, annak hatására hogy a Github Actions SSH-n szól a szervernek, hogy pullolja.
...
Egy yaml (documentation.yaml) kirendereli ezt a dokumentumot és release-be teszi a PDF-et, a másik (docker.yaml)
